...
fn main() {
    unsafe {
        let master_port: mach_port_t = MACH_PORT_NULL;

        let classes_to_match = IOServiceMatching(b"IOFramebufferI2CInterface\0".as_ptr() as *const c_char);
        ...
        let kern_result = IOServiceGetMatchingServices(kIOMasterPortDefault, classes_to_match, &mut matching_services);
        if kern_result != KERN_SUCCESS {
            panic!("ERROR: {}", kern_result);
        }

        loop {
            let io_fb_i2c_service = IOIteratorNext(matching_services);

            if io_fb_i2c_service == MACH_PORT_NULL {
                break;
            }

            let mut props = mem::uninitialized();

            let result = IORegistryEntryCreateCFProperties(io_fb_i2c_service, &mut props, kCFAllocatorDefault, 0);
            if result == KERN_SUCCESS {
                CFDictionaryApplyFunction(props, print_property_entry, ptr::null());
            }

            IOObjectRelease(io_fb_i2c_service);
        }
    }
}
